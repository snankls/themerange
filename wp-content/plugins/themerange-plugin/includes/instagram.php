    ...
    class TS_Instagram {
		...
		function maybe_refresh_token(){	
			...
			if( $need_refresh ){
				...
				if( isset($data['access_token']) ){
					...
					return new WP_Error( 'cant_refresh_token', esc_html__( 'Can not refresh Instagram token. It may be incorrect.', 'themerange' ) );
				}
			}
			return true;
		}
        ...
        function get_user_id(){
			...
			if( isset($response['id']) ){
				...
				return new WP_Error( 'invalid_response', esc_html__( 'Unable to communicate with Instagram.', 'themerange' ) );
			}
		}
        ...
        function get_data_with_token(){
			...
			if( !is_array($response) ){
				return new WP_Error( 'invalid_response', esc_html__( 'Instagram has returned invalid data.', 'themerange' ) );
			}
			...
			foreach( $response as $node ){
				...
				$item['caption'] = isset($node['caption'])?$node['caption']:__('Instagram Image', 'themerange');
				$items[] = $item;
			}
