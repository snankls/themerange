var tr_loading_screen = {
	...
	create: function(){
		...
		var image = jQuery('<img src="' + tr_loading_screen_opt.loading_image + '"  style="max-width: 120px;" />');
			...
			if( !this.image_loaded(this.images[i]) ){
				...
				if( img.offset().top < win_height ){
					...
					img.on('load', function(){
						tr_loading_screen.complete_loading_images();
					});
				}
			}
		}
	...
	,destroy: function(){
		if( !this.destroyed ){
			this.overlay.fadeOut(200, function(){
				tr_loading_screen.overlay.remove();
			});
			this.destroyed = true;
		}
	}
...
tr_loading_screen.create();

jQuery(function(){
	tr_loading_screen.get_images();
});

jQuery(window).on('load', function(){
	tr_loading_screen.destroy();
});